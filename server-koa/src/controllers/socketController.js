  ...
  io.on('connection', (socket) => {
    ...
    socket.on('deployPathUpdated', async (notification) => {
      try {
        ...
        if (!deviceId || !project || !deployPath) {
          console.warn('部署路径更新通知缺少必要参数:', { deviceId, project, deployPath });
          return;
        }
    ...
    socket.on('command:result', async (result) => {
      try {
        ...
        const versionsUpdate = data.version
...
async function queryDeviceVersions(deviceId) {
  try {
    ...
    // 查询前端版本
    const frontendResult = await deviceManager.sendCommand(deviceId, 'getCurrentVersion', {
      project: 'frontend'
    }, 10000); // 10秒超时

    // 查询后端版本
    const backendResult = await deviceManager.sendCommand(deviceId, 'getCurrentVersion', {
      project: 'backend'
    }, 10000);
