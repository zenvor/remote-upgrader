 ...
 */
export function formatDate(value, { timeZone = 'Asia/Shanghai', pattern = 'YYYY-MM-DD HH:mm:ss' } = {}) {
...
export function formatTimeFieldsDeep(payload, options = {}) {
  ...
  const shouldFormatByKey = (key) =>
    typeof key === 'string' && /(At|at|Time|time|Timestamp|timestamp|Date|date|Updated|updated)$/.test(key)
  ...
  const walk = (object) => {
    ...
    if (Array.isArray(object)) {
      for (let i = 0; i < object.length; i++) {
        ...
        const d = toDayjs(v)
        if (d && d.isValid()) {
          object[i] = formatDate(d.toDate(), options)
    ...
    for (const [k, v] of Object.entries(object)) {
      ...
      if ((d && d.isValid() && v instanceof Date) || shouldFormatByKey(k)) {
        ...
        if (tryD && tryD.isValid()) object[k] = formatDate(tryD.toDate(), options)
      } else if (v && typeof v === 'object') {
        walk(v)
      }
    }
