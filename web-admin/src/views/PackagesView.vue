<template>
  <div class="page-container">
    ...
    <a-card :bordered="false" :bodyStyle="{ padding: '0 20px' }">
      <OperationBar :title="'包管理'" :selected-count="0" :total="total" :show-total="true" @refresh="refreshPackages">
        <template #actions>
          <a-select
              ...
              <a-space direction="vertical" size="small" style="width: 100%">
                <a-space align="start" style="width: 100%; justify-content: space-between">
                  ...
                  <span
                    ...
                    style="max-width: 220px; word-break: break-all; text-align: right"
                  >
...
// 数据状态
const { packages, total, filters, loading, fetchPackages, deletePackage: deletePackageAPI } = usePackages()
...
const pkgColumnsAntd = [
  ...
  },
  {
    ...
    customRender: ({ record }) =>
      h(
        'div',
        { class: 'font-mono text-xs text-gray-600 max-w-56 truncate', title: record.packagePath || '-' },
        record.packagePath || '-'
      )
  },
  {
